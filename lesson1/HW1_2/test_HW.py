...
def test_auth_negative(browser):
    browser.get(main_url)
    browser.find_element(By.ID, username_field).send_keys(username_neg)
    browser.find_element(By.ID, password_field).send_keys(password_neg)
    browser.find_element(By.ID, login_button).click()
    assert (browser.find_element(By.XPATH, error_message).text == error_auth_neg), auth_neg_message
...
def test_card_details_from_image(browser):
    test_auth_positive(browser)
    browser.find_element(By.ID, image_link_to_card).click()
...
def test_card_details_from_name(browser):
    test_auth_positive(browser)
    browser.find_element(By.ID, title_link_to_card).click()
    assert browser.current_url == card_url, card_details_message
...
def test_order_positive(browser):
    test_add_card_to_cart_from_catalog(browser)
    browser.find_element(By.CLASS_NAME, shopping_cart_link).click()
    browser.find_element(By.ID, order_checkout_button).click()
    browser.find_element(By.ID, order_firstname_field).send_keys(order_firstname)
    browser.find_element(By.ID, order_lastname_field).send_keys(order_lastname)
    browser.find_element(By.ID, order_zipcode_field).send_keys(order_zipcode)
    browser.find_element(By.ID, order_continue_button).click()
    browser.find_element(By.ID, order_finish_button).click()
    assert browser.current_url == complete_order_url
    assert browser.find_element(By.CLASS_NAME, order_complete_message).text == order_pos_text, order_pos_message
...
def test_logout(browser):
    ...
    browser.find_element(By.ID, logout_button).click()
